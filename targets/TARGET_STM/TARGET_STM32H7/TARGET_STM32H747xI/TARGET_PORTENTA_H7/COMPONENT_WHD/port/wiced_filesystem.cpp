...
MBED_WEAK void wiced_filesystem_mount_error(void) {
    printf("Failed to mount the filesystem containing the WiFi firmware.");
    whd_print_logbuffer();
    while (1) {}
}

MBED_WEAK void wiced_filesystem_firmware_error(void) {
    printf("Please run the \"PortentaWiFiFirmwareUpdater\" sketch once to install the WiFi firmware.");
    whd_print_logbuffer();
    while (1) {}
}
...
static wiced_result_t wiced_filesystem_init()
{
    ...
    {
      mbr_bd = new MBRBlockDevice(qspi_bd, 1);
      if (mbr_bd->init() == QSPIF_BD_ERROR_OK)
      {
          mbr_bd->partition(qspi_bd, 1, 0x0B, 0, 1024 * 1024);
          return WICED_SUCCESS;
      }
    }
    return WICED_ERROR;
...
wiced_result_t wiced_filesystem_file_open(wiced_filesystem_t *fs_handle, wiced_file_t *file_handle_out, const char *filename, wiced_filesystem_open_mode_t mode)
{
    ...
    /* This can be called from user sketch to provide custom block device and mount point*/
    if (wifi_fs == NULL) {
        wiced_filesystem_mount(mbr_bd, 0, fs_handle, WIFI_DEFAULT_MOUNT_NAME);
    }
